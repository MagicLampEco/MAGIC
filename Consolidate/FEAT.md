...
Vault giữ LAMP dưới dạng danh sách `loyalty_holdings` (mỗi entry là `{amount, acquired_epoch, is_locked}`). Mỗi lần vault **nhận thêm LAMP** (người dùng nạp vào) → thêm 1 entry mới (`age=0`). Bản cũ ghi nguồn là "LAMP từ Treasury (reward)" — nguồn đó **không còn tồn tại**: LAMP đứng yên trong vault, không handler nào chuyển LAMP giữa vault và Treasury (I-ACT-7). Ai đọc theo bản cũ sẽ đi tìm một luồng reward không có ở đâu trong hệ. Sau nhiều giao dịch, danh sách phân mảnh → tiêu tốn ExUnit khi xử lý onchain (A5). Giới hạn cứng `MAX_LOYALTY_HOLDINGS = 40` (hạ từ 64 ngày 2026-09-14 — bản cũ nằm TRÊN trần ExUnit vật lý). Neo theo tên hằng, không theo số dòng: `ScheduleGen/onchain/lib/magiclamp/protocol/constants.ak` ▸ `max_loyalty_holdings` ↔ `ScheduleGen/offchain/src/constants.ts` ▸ `MAX_LOYALTY_HOLDINGS`. Khi số entry tiến gần trần, vault có nguy cơ bị khoá (không thể thêm holding mới).
