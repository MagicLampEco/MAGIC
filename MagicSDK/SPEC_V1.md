# MAGIC v1.0 — Spec onchain changes cho Tuân
...
**Scope:** 2 redeemer mới + 1 stub cần implement full. Không đụng vault datum shape (tránh re-build) ngoài việc thêm 2 enum variant.

**Reviewer:** Tuân (cả spec + SDK code).

---

## Tổng quan

| # | Item | Module | Type | Trạng thái hiện tại | Mức |
|---|---|---|---|---|---|
| 1 | `WithdrawLamp { amount }` | SnapshotGen + InstantGen + VacuumGen + ScheduleGen vault | NEW redeemer | Chưa tồn tại | 🔴 Block production |
| 2 | `UpdateProfile { new_profile }` full impl | SnapshotGen vault | EXISTS stub → full | Stub: chỉ check owner sign | 🟡 |
| 3 | `UpdateProfile { new_profile }` thêm vào InstantGen | InstantGen vault | NEW (Option A) | Chưa tồn tại | 🟡 |

**KHÔNG cần thay đổi datum shape.** Vault hash sẽ đổi (vì validator code đổi) → migration plan riêng — xem §6.

---

## §1. `WithdrawLamp { amount: Natural }` — TẤT CẢ 4 vault

### Use case

User rút LAMP unlocked từ vault về ví (hoặc địa chỉ chỉ định). Là 1 chiều ra duy nhất ngoài flow Treasury (Vacuum/Schedule). Không có withdraw thì LAMP locked vĩnh viễn trong vault — UX dealbreaker.
...
WithdrawLamp { amount: Natural } -> {
  ...
  expect output_datum.attribution == input_datum.attribution   // không event attribution cho withdraw

  // W-6: vault output preserves lovelace + holds new LAMP balance
  // (caller có thể giữ min-ADA bằng vault_input_lovelace)
  // Implicit qua datum check W-5 (lamp_balance) — nhưng cũng check value:
...
**Critical:** chọn holding **MỚI NHẤT** (acquired_epoch cao nhất) trước → preserve loyalty của holdings cũ → maximize LF cho Snapshot/Instant sau này. Ngược pattern với `selectLampForLock` (T5 youngest-first lock) về thứ tự nhưng cùng logic "ưu tiên young/new". Locked holdings KHÔNG bao giờ bị touch.

```aiken
// Pseudocode — implement tương tự select_lamp_for_lock trong protocol/lock.ak
pub fn remove_newest_first(
  ...
  // 3. Iterate, consume from newest, partial on last
...
**SDK mirror:** `MagicSDK/src/withdrawLamp.ts:removeNewestFirst` — same algorithm, validator P8 cross-check.
...
| Output vault value khác lamp_balance | W-6 reject |

### Negative tests (Tuân thêm vào `*_only.ts` pattern hiện tại)
...
| Output vault value LAMP ≠ new balance | W-6 reject |
| Wallet không sign | W-2 reject |
| Tx có 2 vault input | W-4 reject |

---

## §2. `UpdateProfile { new_profile: ActivityProfile }` — SnapshotGen full impl
...
UpdateProfile { .. } -> {
  expect list.has(tx.extra_signatories, input_datum.owner)
  True   // ← passes mọi datum tamper. CRITICAL BUG.
}
```

Hiện stub này cho phép user UpdateProfile tamper bất cứ datum field nào (lamp_balance, magic_batches, ...) — bypass mọi A02 check. Phải fix trước v1.0.

### Spec đã có

Spec `vault_profile.ak` partial đã định nghĩa rules — copy logic vào SnapshotGen vault. Toàn bộ §12 (C-PC-V1..V6, T4) đã có offchain reference (`ProfileChange/offchain/src/profile.ts`).
...
UpdateProfile { new_profile } -> {
  ...
  // Vault output value unchanged (no LAMP movement)
  // ... (implicit by lamp_balance check)
  True
}
```
...
`pending_profile` được set ở tx UpdateProfile. Validator **TriggerSnapshot, InstantGen, etc.** sẽ tự áp dụng pending khi `current_epoch >= pending.effective_epoch`:

```aiken
// Add vào helper dùng chung — apply_pending_profile
...
Mỗi validator handler (TriggerSnapshot, InstantGen, …) gọi `apply_pending_profile` trước khi compute M để dùng đúng profile mới.

### Negative tests cho Tuân
...
### Use case (anh đã yêu cầu)

User có 3 khoản LAMP: ngắn / trung / dài hạn. Mỗi khoản 1 vault SnapshotGen riêng. Tiêu LAMP của vault ngắn hạn (qua Withdraw + Instant) **KHÔNG ảnh hưởng tới LF của vault dài hạn** vì LF tính theo `loyalty_holdings` của vault hiện tại (không cross-vault).

### SDK support

`MagicSDK/src/listVaults.ts:listVaultsForOwner` — discover N vault của 1 owner. Trả về danh sách `VaultRecord` kèm `vaultId`, `oldestEpoch`, `lampBalanceOil`, `profile` để PhoenixKey label / sort / chọn.
...
### Khuyến nghị cho PhoenixKey UI

- Cache mapping `userDID → [vaultId₁, vaultId₂, ...]` off-chain
- User UI: "My vaults" list với label do user gán (short/mid/long)
- Mỗi action UX: chọn vault → call SDK với `vaultUtxo` tương ứng

---

## §5. Confirmed quyết định từ anh

| Câu hỏi | Quyết định |
|---|---|
| Withdraw fee | Free, user trả ADA network fee như mọi tx Cardano |
| Session key | Thuộc PhoenixKey, MAGIC không lo. `personal_delegate` field giữ nguyên (reserved) |
| ProfileChange validator riêng? | Không — Option A (gộp vào Snap + Instant vault) |
...
Vault hash đổi → vault cũ stuck ở địa chỉ cũ. Validator cũ vẫn execute (immutable). User có private key + biết validator CBOR cũ → vẫn spend được.

### Lựa chọn

| Hướng | Mô tả | Pros/Cons |
|---|---|---|
| **(a) Burn v0** | Preview chỉ là testnet, drop hết, không support v0 sau khi v1 deploy | Đơn giản. Test users mất tLAMP nhưng testnet rẻ |
| **(b) Withdraw-then-create** | User call `WithdrawLamp` (v0 cũng cần có) → wallet → `createVault` v1 | v0 phải có WithdrawLamp trước → đụng catch-22 (v0 đã deploy, không add được) |
...
| **(d) Don't migrate** | v0 chỉ trên Preview, không lên mainnet | Recommend |

**Recommend hướng (d):**

- v0 (current state) chỉ trên Preview testnet — Tuân test
...
- v0 vault cũ trên Preview stuck — ignore. Lessons learned, không mất production data

### Mainnet launch

Chỉ launch v1.0 (đã có withdraw + full updateprofile). Tuân add 2 redeemer + Aiken test pass → audit → deploy production. Sau deploy, validator hash đóng cứng cho mọi mainnet user.

---

## §7. Implementation checklist cho Tuân
...
### Offchain test (Tuân pattern hiện tại)
...
- ✅ `MagicSDK/src/withdrawLamp.ts` — SDK function complete, `removeNewestFirst` algorithm matches §1 spec
- ✅ `MagicSDK/src/updateProfile.ts` — SDK function complete, cooldown + lazy apply
- ✅ `MagicSDK/src/listVaults.ts` — multi-vault discovery (works on current onchain, no v1 dep)
- ✅ Unit tests 18/18 pass

### Redeemer constructor indices đề xuất

Em điều chỉnh nếu cần khác. SDK hiện đang dùng:
...
Nếu em thay đổi index → update `MagicSDK/src/withdrawLamp.ts:WITHDRAW_LAMP_CONSTR_INDEX` và `updateProfile.ts:UPDATE_PROFILE_CONSTR_INDEX` tương ứng.
