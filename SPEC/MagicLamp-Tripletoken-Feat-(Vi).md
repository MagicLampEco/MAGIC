...
usage_factor_q    = usage_factor_floor_q + ⌊ (Q − usage_factor_floor_q) × usage_ratio_q / Q ⌋
scale_limit       = ⌊ Σ_window generated × Q / (horizon × scale_coverage_q) ⌋      -- §6.1.2, horizon ở §6.1.4
...
> **CHƯA CHỐT: `CC-GEN-GB-ROLE` · treo:** `GB` là TRẦN (`min`) hay là yếu tố QUYẾT ĐỊNH độ lớn lượng sinh. Quyết định 2026-09-17 nói hai cửa "dựa trên" thặng dư GreenBack. Với dạng `min` dưới đây, **`GB` chỉ tác động khi thặng dư gần cạn**: lúc dồi dào, lượng sinh hoàn toàn do `L` và `usage_ratio` quyết. **Ràng buộc TẠM:** `GB` là trần + cổng, không nhân (fail-closed: không để `GB` nâng suất mỗi LAMP).
...
- **Khởi động lạnh: `Σ generated = 0` ⟹ `usage_ratio_q = 0` ⟹ `usage_factor_q = usage_factor_floor_q`.** Vì sao không cho vault mới hệ số đầy: nếu vault mới hưởng nhiều hơn vault có lịch sử kém, đóng-rồi-mở-lại vault là cách xoá lịch sử có lợi — và `INV-ONE-PERSON-ONE-VAULT` (`BOUNDARIES.md`) chưa được ép. Hạt giống `wakeme_seed_credit` không đi vào cửa sổ đếm. **CHƯA CHỐT: `CC-GEN-COLD-START` · treo:** vault mới ở sàn hay ở mức trung tính (ví dụ hệ số giữa dải) · **TẠM:** sàn.
...
- **Fail-closed, không giá trị mặc định:** thiếu beacon · nhiều hơn một beacon · sai NFT/sai script · cũ hơn ngưỡng · cờ `depeg` bật ⟹ giao dịch sinh bị từ chối.
...
**Thời điểm tính `L`.** LAMP nạp vào vault giữa epoch tính NGAY vào `lamp_available` (chốt 2026-07-30, §6.3). **CHƯA CHỐT: `CC-GEN-L-TIMING` · treo:** giữ "tính ngay" hay đòi LAMP nằm đủ lâu. **TẠM:** tính ngay. **Rủi ro đang mở:** thuê LAMP vài phút cuối epoch, sinh, rồi trả LAMP ở epoch kế — `INV-INSTANT-LOCK` chỉ giữ LAMP tới hết epoch nên chi phí thuê chỉ là vài phút; lượng MAGIC thu được sống tới hết epoch đó.
...
**Hai phanh bổ sung:**
- Cờ `depeg` trên beacon bật ⟹ không sinh (§6.1.3).
...
> **CHƯA CHỐT: `CC-GEN-SCHEDULE-FIXED` · treo:** (1) lượng/epoch của ScheduleGen cố định tuyệt đối từ lúc ký hay được giảm. Nguyên văn quyết định 2026-09-17: đặt ScheduleGen để *"luôn luôn có được 1 lượng MAGIC cố định qua mỗi epoch … và nó không thay đổi bất kỳ điều gì"* — **mâu thuẫn trực tiếp** với bậc cứu (1) ở trên và `INV-LOCKED-RATE-CAPPED` (§12), hai cơ chế cho lượng/epoch **giảm** khi GreenBack thiếu hoặc khi `ρ_e` hạ; (2) fire có dừng khi beacon báo `depeg` hoặc `GB = 0` không (hiện fire không đọc beacon). **Ràng buộc TẠM:** giữ nguyên hai cơ chế giảm (chỉ được GIẢM, không bao giờ TĂNG so với lúc ký); fire không đọc beacon — fail-closed về phía cung MAGIC, và chấp nhận rằng câu "không thay đổi bất kỳ điều gì" chưa được thoả.
...
| **INV-MAGIC-CITIZEN** | Lượng sinh từ LAMP (InstantGen + ScheduleGen) = `min(amount_by_lamp, GB_available)` (§6.1.1): `L` là cơ sở nhân, `usage_ratio` (consumed/generated, 6 epoch đã qua, không gồm hết hạn) là hệ số nhân dải `[0.5, 1]`, phần hệ số trên sàn chỉ áp tới `scale_limit`, `GB` là trần + cổng; với ScheduleGen, trần gộp toàn mạng là cổng `κ` (§6.1.4). MAGIC đang cầm không vào công thức. Người tiêu 0 vẫn sinh ở sàn; người không nắm LAMP sinh 0. VP C1 (§10) vẫn keyed MAGIC-đã-tiêu cross-DID |
| **INV-CASHBACK-BOUND** | hoàn-tiền/ưu-đãi-phí mỗi DID ≤ MAGIC thật đã tiêu; KHÔNG áp cho lượng sinh từ LAMP (§6.3) |
| **INV-INSTANT-LOCK** | InstantGen khoá `⌈m × Q × Q / (generation_rate_q × usage_factor_q)⌉` LAMP (`instant_locked`) tới hết epoch; khoá chỉ hết theo chuyển epoch qua `instant_lock_epoch` (§6.3, §6.1.4). Mã hiện tại chưa hiện thực |
...
| **INV-LOCKED-RATE-CAPPED** | T8 giữ đúng chữ (suất đã ký không TĂNG) bằng trần trôi: `M_i = ⌊min(λ·rate_locked_q/Q, λ·ρ_e/Q) × usage_factor_locked_q / Q⌋` (§6.1.4). Chặn đòn khoá `L=200` ngay trước khi hạ suất để giữ suất cũ 2,7 năm |
...
- `CC-GEN-GB-ROLE` · `GB` là trần (`min`) hay là yếu tố quyết định độ lớn lượng sinh (§6.1.1) · TẠM: trần + cổng, không nhân.
- `CC-GEN-SCHEDULE-FIXED` · lượng mỗi epoch của hợp đồng ScheduleGen cố định tuyệt đối hay giảm được; fire khi depeg (§6.1.4) · TẠM: giữ hai cơ chế giảm (vế `min` với `ρ_e` của `INV-LOCKED-RATE-CAPPED` và `usage_factor_locked_q` lưu lúc ký), fire không đọc beacon.
- `CC-GEN-COLD-START` · vault mới ở sàn hay ở mức trung tính (§6.1.2) · TẠM: sàn.
- `CC-GEN-L-TIMING` · LAMP nạp giữa epoch tính ngay hay phải nằm đủ lâu (§6.1.4) · TẠM: tính ngay; rủi ro thuê chớp nhoáng cuối epoch còn mở.
