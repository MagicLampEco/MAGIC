...
MagicBatch {                                      // 9 TRƯỜNG — đếm lại trước khi encode
  ...
  halved              : Bool,        // TRƯỜNG CHẾT — luôn False dưới mô hình cliff.
                                     // Giữ để hình dạng 9 trường còn tương thích byte với
                                     // datum đã deploy. Mọi giá trị True bị từ chối.
}
```
...
### §7.4 Bất biến validator `consume` (C-CM-1..8)

> Bảng dưới liệt C-CM-1..5. Validator hiện ép thêm **C-CM-6/7/8**; nguồn đầy đủ và đang đúng
